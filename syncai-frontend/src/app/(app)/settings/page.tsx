...
const THEME_OPTIONS: { value: string; label: string; icon: React.ElementType; desc: string; disabled?: boolean }[] = [
...
function ProfileTab({ me }: { me: { name?: string; email?: string } | null }) {
  return (
    <div className="space-y-3">
      <div className="rounded-2xl p-px" style={{ background: "var(--gradient-accent)" }}>
        <div className="rounded-[15px] px-5 py-5 flex items-center gap-4" style={{ background: "var(--bg-surface)" }}>
          <div
            className="w-14 h-14 rounded-2xl flex items-center justify-center text-white text-xl font-bold shrink-0"
            style={{ background: "var(--gradient-accent)", boxShadow: "0 8px 24px rgba(99,102,241,0.35)" }}
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-1">
              ...
              <p className="text-[12px] truncate" style={{ color: "var(--text-muted)" }}>{me?.email ?? "—"}</p>
            </div>
          </div>
          <span
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        {[
          ...
          <div key={row.label} className="flex items-center justify-between px-5 py-3.5" style={{ borderBottom: "1px solid var(--border)" }}>
            <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>{row.label}</p>
            <div className="flex items-center gap-2">
              {row.note && (
                <span className="text-[11px] px-1.5 py-0.5 rounded" style={{ background: "var(--bg-soft)", color: "var(--text-faint)" }}>
                  {row.note}
                </span>
              )}
              <p className="text-[13px] font-medium" style={{ color: "var(--text)" }}>{row.value}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        {THEME_OPTIONS.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[12px] font-medium" style={{ color: isActive ? "var(--accent)" : "var(--text-soft)" }}>
                {opt.label}
              </span>
              {isActive && (
    ...
    <div className="space-y-3">
      <div className="rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
        <div className="flex items-center gap-3.5 px-5 py-4" style={{ borderBottom: "1px solid var(--border)" }}>
          ...
          <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full shrink-0" style={{ background: "rgba(34,197,94,0.12)", color: "var(--green)" }}>
            활성
          </span>
        </div>
        <div className="px-5 py-4">
          <p className="text-[12px] mb-3" style={{ color: "var(--text-muted)" }}>로그아웃하면 현재 세션이 종료됩니다.</p>
  ...
  return (
    <main className="flex-1 overflow-y-auto" style={{ background: "var(--bg-base)" }}>
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="mb-7">
          <h1 className="text-[20px] font-bold tracking-tight" style={{ color: "var(--text)" }}>설정</h1>
          <p className="text-[13px] mt-0.5" style={{ color: "var(--text-muted)" }}>계정 및 앱 환경 관리</p>
        </div>

        <div className="flex gap-4 items-start">
          <nav className="w-40 shrink-0 rounded-2xl overflow-hidden" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)" }}>
            {TABS.map((tab) => {
        ...
        <p className="text-center text-[11px] mt-10" style={{ color: "var(--text-faint)" }}>
          SyncAI &middot;{" "}
          <a href="mailto:support@syncai.dev" className="hover:underline">support@syncai.dev</a>
        </p>
      </div>
    </main>
  );
